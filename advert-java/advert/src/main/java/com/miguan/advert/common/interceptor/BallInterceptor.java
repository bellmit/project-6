package com.miguan.advert.common.interceptor;
...
//	private final String ERROR_NEW_DEVICE= "701";  //根据token查询不到用户(您的账号在其他设备上登录，请重新登录)
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		*//*if(!isOutAccessLimit(request,response,handler)) {
		...
		//在 token 开启且不是正式环境下，允许访问 doc.html
		if(url.indexOf("doc") >= 0 && !"prod".equals(appEnvironment)) {
			return true;
		}
		...
		if(StringUtils.isBlank(token) && StringUtils.isBlank(authToken)) {
			*//* 接口传来的 token为空 并且后台 authToken 也为空，那么授权不通过 *//*
			*//* 后台 authToken 交给 shiro 取验证 *//*
