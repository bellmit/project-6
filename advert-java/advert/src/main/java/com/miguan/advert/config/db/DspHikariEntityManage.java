...
@EnableJpaRepositories(
        entityManagerFactoryRef = "dspEntityManagerFactory",
        transactionManagerRef = "dspTransactionManager",
        basePackages = {"com.miguan.advert.repositories"}
)
public class DspHikariEntityManage {

    @Resource
    private HikariDataSource dspDatasource;

    @Autowired
    private Environment environment;

    @Bean(name = "dspEntityManager",destroyMethod = "close")
    public EntityManager entityManager(EntityManagerFactoryBuilder builder){
        return entityManageFactory(builder).getObject().createEntityManager();
    }

    @Bean(name = "dspEntityManagerFactory")
    public LocalContainerEntityManagerFactoryBean entityManageFactory(EntityManagerFactoryBuilder builder){
        LocalContainerEntityManagerFactoryBean entityManagerFactory =  builder.dataSource(dspDatasource)
                .packages("com.miguan.advert.domain.entity").persistenceUnit("DspHikariEntityManage").build();
