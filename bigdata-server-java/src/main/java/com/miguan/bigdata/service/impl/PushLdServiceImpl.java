...
@Service
@Slf4j
public class PushLdServiceImpl extends BaseAutoPushService implements PushLdService {
     ...
     */
    @Override
    public List<PushResultVo> findLdAutoPushList(Integer pushId, Integer userType, String packageName, String dd, Integer pageNum, Integer pageSize) {
        ...
        List<PushResultVo> pageList = null;
    ...
    @Override
    public List<PushResultVo> findLdAutoPushList(PushDto pushDto) {
    ...
    public void syncLdVideoInfo() {
        String maxUpdateTime = pushLdMapper.findMaxLdVideoUpdatedTime();
        maxUpdateTime = (maxUpdateTime == null ? "0000-00-00 00:00:00" : maxUpdateTime);

        int count = pushLdMapper.countNewLdVideos(maxUpdateTime);
        int index = 0;
        int pageSize = 5000;
        ...
        while (index < count) {
            params.put("startRow", index);
            List<LdVideosVo> dataList = pushLdMapper.queryNewLdVideos(params);  //在mysql查询出视频数据

            if(dataList != null && !dataList.isEmpty()) {
                pushLdMapper.deleteLdVideoInfoById(dataList);   //删除clickhouse中已经存在的来电秀视频数据
                pushLdMapper.batchInsertUpdateLdVideo(dataList);  //重新同步最新的来电秀数据到clickhouse中
            }
            index = index + pageSize;
