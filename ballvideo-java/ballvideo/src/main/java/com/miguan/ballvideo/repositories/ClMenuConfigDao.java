...
public interface ClMenuConfigDao extends JpaRepository<ClMenuConfig, Long> {
     ...
     * state    状态：0启用  1禁用
     **/
    @Query(value = "select * from cl_menu_config where app_package = ?1 and state = '0' and IFNULL(hide_channel,'0') not like CONCAT('%',?2,'%') order by sort asc",nativeQuery = true)
    ...
    @Query(value = "select * from cl_menu_config where app_package = ?1 and state = '0' and IFNULL(hide_channel,'0') not like CONCAT('%',?2,'%') and " +
