...
public class ToolMofangServiceImpl implements ToolMofangService {
    ...
    public List<ChannelGroup> getChannelGroups(String appPackage) {
        String nativeSql = "select a.app_type as appPackage,a.msg_sign as msgSign,a.name from channel_group a where a.status=1 and a.app_type = ?";
    ...
    public void ChannelInit() {
        if (redisService.exits(Channel.CHANNEL_REDIS)) {
    ...
    @Override
    public List<MessageModel> getMessageModelByModelId(String messageModelId) {
        String nativeSql = "SELECT * from message_model where id= '" + messageModelId + "'";
        List<MessageModel> messageModelList = dynamic4Query.nativeQueryList(MessageModel.class,nativeSql);
        return messageModelList;
    }

    @Override
    public List<MessageDictionary> getMessageDictionaryByModelId(String messageModelId) {
        String nativeSql = "SELECT dict.id,dict.project_id,dict.column_name,dict.english_field,dict.chinese_explain from message_model aa LEFT JOIN message_dictionary dict on aa.project_id  = dict.project_id " +
        "where aa.id= '" + messageModelId + "'";
        List<MessageDictionary> messageDictionaryList = dynamic4Query.nativeQueryList(MessageDictionary.class,nativeSql);
