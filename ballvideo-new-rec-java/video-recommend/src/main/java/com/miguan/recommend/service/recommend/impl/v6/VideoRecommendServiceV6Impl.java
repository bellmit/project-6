...
import com.miguan.recommend.bo.UserCatScoreDto;
...
@Slf4j
@Service(value = "videoRecommendServiceV6")
public class VideoRecommendServiceV6Impl extends AbstractRecommendService implements VideoRecommendService<VideoRecommendDto> {
...
//            }
        ...
        log.debug("{} 推荐0.6 确定分类的视频ID>>{}", baseDto.getUuid(), JSONObject.toJSONString(specificCatVideos));
        // 热度视频返回的数量
        ...
        log.debug("{} 推荐0.6 根据排序获取的视频ID>>{}", baseDto.getUuid(), JSONObject.toJSONString(videoSortList));
        ...
        log.warn("推荐0.6 此次给uuid({})推荐了 {} 个视频，推荐视频ID集：{}, 激励视频 {} 个, 激励视频ID：{}",
    ...
    public List<Object> getAsync(BaseDto baseDto, VideoRecommendDto recommendDto) {
        ...
        log.warn("{} 推荐0.6 召回时长：{}，召回数量：{}，其中热门召回：{}", baseDto.getUuid(), (pt2 - pt1), allVideos.size(), catSum);
        ...
        recommendDto.getJlvideo().addAll(jlVideo.stream().map(IncentiveVideoHotspot::getVideo_id).collect(Collectors.toList()));
        recommendDto.setJlvideoCat(jlVideo.stream().collect(Collectors.toMap(IncentiveVideoHotspot::getVideo_id, IncentiveVideoHotspot::getCatid)));
        log.info("{} 推荐0.6 排序总时长：{}", baseDto.getUuid(), (pt3 - pt2));
    ...
    public CompletableFuture<Void> findHotVideo(BaseDto baseDto, VideoRecommendDto recommendDto, int getNum, Map<Integer, List<VideoHotspotVo>> userCatVideo) {
        ...
        Function<Integer[], Number> f = e -> {
            ...
            if (isEmpty(videoId1)) {
                log.debug("{} 推荐0.6 获取用户兴趣分类[{}]视频：{} 个", baseDto.getUuid(), e[0], 0);
            } else {
                log.debug("{} 推荐0.6 获取用户兴趣分类[{}]视频：{} 个", baseDto.getUuid(), e[0], videoId1.size());
                userCatVideo.put(e[0], videoId1);
            }
            log.debug("{} 推荐0.6 获取用户兴趣分类视频时长：{}", baseDto.getUuid(), (System.currentTimeMillis() - pt));
    ...
    public void initRecommendParam(BaseDto baseDto, VideoRecommendDto recommendDto, JedisPool recDB9Pool, MongoTemplate logMongTemplate, VideosCatService videosCatService) {
        ...
        log.info("{} 推荐0.6 默认分类>>{}", baseDto.getUuid(), JSONObject.toJSONString(defaultCat));
        List<Integer> userCat = this.getUserCat(baseDto, recommendDto.getDefaultCatList());
        ...
        log.info("{} 推荐0.6 实时分类>>{}", baseDto.getUuid(), JSONObject.toJSONString(userCat));

        List<Integer> recommendCat = new ArrayList<Integer>();
        if (baseDto.getPublicInfo().isNewApp()) {
            // 新用户默认渠道分类 + 两个实时分类 +  第一默认分类的相似分类
            Integer firstCat = recommendDto.getUserCats().size() > 0 ? recommendDto.getUserCats().get(0) : defaultCat.get(0);
            List<Integer> similarCat = super.getSimilarCat(firstCat, recommendDto.getExcludeCatList(), baseDto.isABTest(), logMongTemplate, videosCatService);
            if (!isEmpty(similarCat)) {
                similarCat.removeAll(defaultCat);
                similarCat.removeAll(recommendDto.getUserCats());
                recommendDto.setSimilarCats(similarCat);
            }
            log.info("{} 推荐0.6 作为新用户的相似分类>>{}", baseDto.getUuid(), JSONObject.toJSONString(similarCat));

            recommendCat.addAll(recommendDto.getUserCats().subList(0, Math.min(2, recommendDto.getUserCats().size())));
            recommendCat.addAll(defaultCat.subList(0, Math.min(1, defaultCat.size())));
            ...
            List<Integer> similarCat = super.getSimilarCat(recommendCat.get(0), recommendDto.getExcludeCatList(), baseDto.isABTest(), logMongTemplate, videosCatService);
            if (!isEmpty(similarCat)) {
                if (!isEmpty(offlineCat)) {
            ...
            log.info("{} 推荐0.6 作为老用户的相似分类>>{}", baseDto.getUuid(), JSONObject.toJSONString(similarCat));
        }

        List<Integer> finalCatList = recommendCat.stream().distinct().limit(6).collect(Collectors.toList());
        log.info("{} 推荐0.6 最终召回分类>>{}", baseDto.getUuid(), JSONObject.toJSONString(finalCatList));
...
//        }
    ...
    private List<Integer> getUserCatBySorce(BaseDto baseDto, List<Integer> defaultCat) {
        ...
        if (isEmpty(map)) {
            log.debug("{} 推荐0.6 用户兴趣分类评分为空，直接返回默认分类", baseDto.getUuid());
            return Lists.newArrayList(defaultCat);
        }

        log.debug("{} 推荐0.6 用户兴趣分类评分:{}", baseDto.getUuid(), map);
        ...
        log.info("{} 推荐0.6 排序后的用户兴趣分类评分:{}", baseDto.getUuid(), map);
        return catScoreList;
    }
    ...
    private void addTheFirstVideoOfTopCat(BaseDto baseDto, List<Integer> catList, Map<Integer, List<String>> catVideoListMap, List<String> recVideoList, int topCatNum) {
        ...
        for (int i = 0; i < cycleSize; i++) {
            ...
            log.info("{} 推荐0.6 根据最感兴趣分类>>{}，获取到视频>>{}", baseDto.getUuid(), catid, videoId);
