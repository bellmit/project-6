...
public class MySqlSecondDBConfig {
    ...
    @Bean("threeProperties")
    @ConfigurationProperties(prefix = "spring.threedatasource")
    public DataSourceProperties sixProperties(){
        return new DataSourceProperties();
    }


    @Bean(name = "threeDataSource")
    public HikariDataSource sixDataSource(@Qualifier(value = "threeProperties") DataSourceProperties dataSourceProperties) {
        HikariDataSource ds = dataSourceProperties.initializeDataSourceBuilder().type(HikariDataSource.class)
                .build();
