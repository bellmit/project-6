...
public class FileUtil {
     ...
     * @return
     */
    public static boolean isImageOrVideo(String fileType) {
        return "jpeg".equalsIgnoreCase(fileType) || "jpg".equalsIgnoreCase(fileType) || "png".equalsIgnoreCase(fileType) || "gif".equalsIgnoreCase(fileType)|| "mp4".equalsIgnoreCase(fileType) || "mov".equalsIgnoreCase(fileType) || "avi".equalsIgnoreCase(fileType);
    }
     ...
     * @since 1.0.0
     */
    public static UploadFileModel upload(MultipartFile file, String prefix, String folder, int size, String uploadType) {
        ...
        if (picSize.compareTo(size * 1024 * 1024L) > 0) {
            log.error("文件超出"+size+"M大小限制");
            model.setErrorMsg("文件超出"+size+"M大小限制");
            return model;
        }
        // 保存文件
        String s = "/";
        String filePath = "data" + s + uploadType + s + folder + s +
                DateUtil.dateStr(DateUtil.getNow(), DateUtil.DATEFORMAT_STR_013) + s + picName;
        if(TypeConstant.UPLOAD_FILE_TYPE_IMAGE.equals(uploadType)){
            filePath = uploadToAws(fi, filePath);
        } else {
            filePath = uploadToAwsVideo(fi, filePath,file.getContentType());
        }
     ...
     * @param filePath
     */
    private static String uploadToAwsVideo(DiskFileItem fi, String filePath, String contentType) {
        try {
