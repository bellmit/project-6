...
@ApiModel("轩辕广告位表")
@Data
public class XyAdPositionDto extends BaseModel {
    ...
    @Max(value = 999999999,message = "展示上限(时)不能超过10位!")
    private Long showLimitHour;

    @ApiModelProperty("同一个用户，1天内最多展示限制")
    @Max(value = 999999999,message = "展示上限(天)不能超过10位!")
    private Long showLimitDay;

    @ApiModelProperty("同一个用户，前后两次请求广告的间隔秒数")
    @Max(value = 999999999,message = "展示间隔(秒)不能超过10位!")
