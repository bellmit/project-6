 ...
 * @Date 2021/1/21
 **/
@Api(value = "广告位(后台)控制层", tags = {"广告位(后台)接口"})
@RestController
@Slf4j
@RequestMapping("/api/back/xyAdPosition")
public class XyAdPositionBackController extends AuthBaseController {
    ...
    @GetMapping("/pageList")
    public ResultMap<PageInfo<XyAdPositionVo>> pageList(@ApiParam("媒体账号名称") String username,
                                                       @ApiParam("类型:1.广告位名称,2.广告位key,3.应用名称,4.应用key") Integer type,
        ...
        return ResultMap.success(service.pageList(XyConstant.BACK_PLAT,null,username,type,keyword, clientType ,status ,adType , pageNum, pageSize));
    }

    @ApiOperation("查询广告位列表(已启用)")
    @GetMapping("/list")
    public ResultMap<List<XyAdPositionSimpleVo>> list(@ApiParam("媒体账号的id") Long userId,@ApiParam("应用id") Long appId) {
        return ResultMap.success(service.findList(XyConstant.BACK_PLAT,userId,appId));
    }

    @ApiOperation("新增、编辑广告位")
    @PostMapping("/save")
    @LogInfo(pathName="后台-广告位管理-新增、编辑广告位",plat= XyConstant.BACK_PLAT,type = XyConstant.LOG_SAVE)
    public ResultMap save(@ApiParam("广告位实体类") @RequestBody @Valid XyAdPositionDto xyAdPositionDto) {
        try {
            //todo
            XyAppDetailVo appInfo = xyAppService.findById(xyAdPositionDto.getAppId());
            if(appInfo != null){
                xyAdPositionDto.setUserId(appInfo.getUserId());
            }
    ...
    @ApiOperation("删除广告位")
    @GetMapping("/delete")
    @LogInfo(pathName="后台-广告位管理-删除广告位",plat= XyConstant.BACK_PLAT,type = XyConstant.LOG_DELETE)
    ...
    @ApiOperation("获取/媒体/应用/广告位联动选择项")
    @GetMapping("/thirdlinkageSelection")
    public ResultMap thirdlinkageSelection() {
        try {
            List<OptionsVo> optionsVo = service.thirdlinkageSelection(XyConstant.BACK_PLAT);
