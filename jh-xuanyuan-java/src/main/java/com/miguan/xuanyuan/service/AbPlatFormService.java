...
import com.miguan.xuanyuan.dto.ab.AbExperiment;
import com.miguan.xuanyuan.dto.ab.AbFlowDistribution;
import com.miguan.xuanyuan.dto.ab.AbLayer;
import com.miguan.xuanyuan.dto.ab.AdAdvertFlowConfig;
...
public class AbPlatFormService {
    ...
    @Value("${api.abtest.url}")
    private String url;

    @Value("${api.abtest-php.url}")
    private String php_url;

    @Value("${api.abtest-php.userName}")
    private String userName;

    @Value("${api.abtest-php.password}")
    ...
    //通过包名获取appid
    private static final String API_APP_PACKAGE = "/api/app/getAppInfoByPackage";
    ...
    public ResultMap getExpById(String expId){
        String errStr = "";
        try {
            String api = url.concat(API_ROUTE_URL).concat("?exp_id=").concat(expId);
            ResponseEntity<ResultMap> res = restTemplate.getForEntity(api, ResultMap.class);
            if (res.getStatusCode().is2xxSuccessful()) {
                ResultMap resD = res.getBody();
                return resD;
            }
        } catch (HttpClientErrorException e) {
            errStr = "xuanyuan调用AB实验平台接口失败，客户端调用异常";
            log.error(errStr + "：{}", e.getMessage(), e);
        } catch (HttpServerErrorException e) {
            errStr = "xuanyuan调用AB实验平台接口失败，服务端异常";
            log.error(errStr + "：{}", e.getMessage(), e);
    ...
    public AbResultMap getAppInfoByPackage(String appPackage, String token) {
        Map<String,String> params = Maps.newHashMap();
        params.put("app_package", appPackage);
        return getUrl(params, token, API_APP_PACKAGE);
    }
    ...
    public ResultMap<Map<String,Object>> getAbFlowMapByInt(String abFlowId) {
        ...
        if(MapUtils.isNotEmpty(expInfo)){
            if (!expInfo.get("code").startsWith(StrategyGroupConstant.AB_EXP_CODE_PREFIX)) {
