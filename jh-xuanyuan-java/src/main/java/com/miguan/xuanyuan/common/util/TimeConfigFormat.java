...
public class TimeConfigFormat {
     ...
     * @param timesConfig
     */
    public static String analysisTimeConfig(Integer timeSetting,String timesConfig) throws CommonException {
        if(XyConstant.TIME_SETTING_APPOINT == timeSetting){
            return timesConfig;
        } else if (XyConstant.TIME_SETTING_APPOINT_MANY == timeSetting){
            //这算是校验没过关了 防止垃圾数据
            if(StringUtils.isEmpty(timesConfig)){
                throw new CommonException("您所选的投放时间类型为指定多个时段,但是却未选择时间段.");
            }
