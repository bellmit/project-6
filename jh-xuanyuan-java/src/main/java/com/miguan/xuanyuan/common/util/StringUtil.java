...
//	}
	...
	/**
	 * 驼峰转下划线,最后转为大写 （首字母是大写的）
	 * @param str
	 * @return
	 */
	public static String humpToLineWithodHead(String str) {
		if(str == null) {
			return null;
		}
		String firstlowerStr = StringUtil.firstCharLowerCase(str);
		Matcher matcher = humpPattern.matcher(firstlowerStr);
		StringBuffer sb = new StringBuffer();
		while (matcher.find()) {
			matcher.appendReplacement(sb, "_" + matcher.group(0).toLowerCase());
		}
	...
	public static List<Integer> strToIntegerList(String str){
		if(StringUtils.isEmpty(str))return null;
		String [] arrA = str.split(",");
		List<Integer> arrInteger = Lists.newArrayList();
		Arrays.asList(arrA).forEach(s -> {
			if(StringUtils.isNotEmpty(s)) arrInteger.add(Integer.parseInt(s));
		});
		return arrInteger;
	}
    ...
    public static String subLastPoint(String str){
